...
namespace InGame.MySystem.Game.Handler
{
    ...
    public class PieceSocketEventHandler : BaseSocketEventHandler
    {
        ...
        public override void OnConnect()
        {
            ...
            NetworkManager.Instance.Socket.On("attackedPiece", (data) =>
            {
                ...
                MainThreadDispatcher.Enqueue(() =>
                {
                    ...
                    _ = InGameContext.Current.Data.PieceManager.AttackRelatedPiecesMove(returnPiece, attackPiece, returnParent, attackParent, setInfo.returnPos, setInfo.attackPos, setInfo.isFirePowerAttack); // 공격 받은 기물 및 공격한 기물 이동 함수
                });
            });
        }
        ...
        {
            NetworkManager.Instance.Socket.Off("setPiece");
            NetworkManager.Instance.Socket.Off("attackedPiece");
        }
    }
}
