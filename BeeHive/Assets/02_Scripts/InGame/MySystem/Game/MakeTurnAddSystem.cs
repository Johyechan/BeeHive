    ...
    public class MakeTurnAddSystem
    {
            ...
            if(GameModeManager.Instance.CurrentGameMode.IsTutorial())
            {
                switch (TutorialManager.Instance.CurrentTutorialState)
                {
                    case TutorialState.Turn1_Player: // 첫 번째 턴(플레이어 턴)
                        await WalletEvent.OnGetGoldBar?.Invoke(2); // 금괴 2개 획득
                        break;
                    case TutorialState.Turn1_AI: // 첫 번째 턴(AI 턴)
                        await WalletEvent.OnGetGoldBar?.Invoke(3); // 금괴 3개 획득
                        break;
                    default: // 다른 상태에서는
                        await WalletEvent.OnGetGoldBar?.Invoke(2); // 금괴 2개 획득
                        break;
        ...
        private async Task GetRoad()
        {
            ...
            else // 튜토리얼 일 때
            {
                ...
                await PieceEvents.OnGetRoad?.Invoke(2, InGameContext.Current.Data.TurnManager.CurrentTeamType, roadParent); // 도로 2개 획득(현재 턴의 팀)
            }
