    ...
    public class Miner : PieceBase
    {
        ...
        public override void ObjectClicked()
        {
            ...
            if(!_isSelected) // 선택된 상태가 아닐 경우
            {
                ...
                HighLightEvents.OnPiecePlacementHighLight?.Invoke(false, true); // 하이라이트 끄기, 배치 가능 배치 판 대상
                HighLightEvents.OnPieceMovementHighLight?.Invoke(true, false); // 하이라이트 키기, 이동 가능 배치 판 대상
