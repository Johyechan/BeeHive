...
namespace InGame.MyObject.Handler
{
    ...
    public class RoadPlaceReturnCheckHandler
    {
        ...
        public async Task<bool> IsReturn(int leftPieceCount, int cost)
        {
            ...
            if (!await WarningEvent.OnCheckLeftPieceCount(leftPieceCount, "남은 도로가 없어 배치할 수 없습니다")) // 남은 도로가 없다면
            {
                HighLightEvents.OnRoadPlacementHighLight?.Invoke(false); // 도로 칸 하이라이트를 끄는 매개변수로 이벤트 콜
                return true; // 반환
            }

            if (!await WarningEvent.OnCanPayCost.Invoke(cost, "금괴가 부족하여 도로를 배치할 수 없습니다.")) // 비용을 지불할 수 없다면
            {
                HighLightEvents.OnRoadPlacementHighLight?.Invoke(false); // 도로 칸 하이라이트를 끄는 매개변수로 이벤트 콜
                return true; // 반환
            }
