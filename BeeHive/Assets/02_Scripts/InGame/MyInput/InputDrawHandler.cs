...
namespace InGame.MyInput
{
    ...
    public class InputDrawHandler
    {
        ...
        private async Task Draw()
        {
            ...
            NetworkManager.Instance.Socket.Emit("debug", "카드 드로우 (Deck: 38)");
            ...
            switch (TeamManager.Instance.CurrentTeamType)
            {
                case TeamType.Team1: // 현재 팀이 Team1일 때
                    await DOTween.Sequence()
                          ...
                          .JoinCallback(() => DrawEventSystem.OnCardObjectSet?.Invoke(_deck.player1CardsParent)).AsyncWait();// 드로우 이벤트 인보크 후 시퀀스 완료 시 Task 완료 반환
                    break;
                case TeamType.Team2: // 현재 팀이 Team1일 때
                    await DOTween.Sequence()
                          ...
                          .AppendCallback(() => DrawEventSystem.OnCardUISet?.Invoke())
                          .JoinCallback(() => DrawEventSystem.OnCardObjectSet?.Invoke(_deck.player2CardsParent)).AsyncWait(); // 드로우 이벤트 인보크 후 시퀀스 완료 시 Task 완료 반환
                    break;
                case TeamType.Team3: // 현재 팀이 Team1일 때
                    await DOTween.Sequence()
                          ...
                          .AppendCallback(() => DrawEventSystem.OnCardUISet?.Invoke())
                          .JoinCallback(() => DrawEventSystem.OnCardObjectSet?.Invoke(_deck.player3CardsParent)).AsyncWait(); // 드로우 이벤트 인보크 후 시퀀스 완료 시 Task 완료 반환
                    break;
            }
