    ...
    public class PieceButton : PlaceUIButton
    {
        ...
        public override void OnUIButtonClick()
        {
            ...
            if (!_isHighLightOn) // 하이라이트가 꺼져 있을 때
            {
                ...
                foreach (var piece in PlacePlaneManager.Instance.HighLightHandler.CanPiecePlacePlanesProp) // 배치 가능한 기물 칸들 순회
                {
                ...
                if(HighLightEvents.SelectedPlacementType != _canPlaceType) // 만약 현재 배치 가능한 타입이 다르다면
                {
                    ...
                    HighLightEvents.OnPiecePlacementHighLight?.Invoke(true, true); // 배치 가능한 기물 칸 하이라이트 키기(하이라이트 키기 여부, 배치 칸 이동 칸 여부 - true는 배치칸, false는 이동칸)
