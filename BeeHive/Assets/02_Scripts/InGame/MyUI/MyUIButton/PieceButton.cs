        ...
        public override void OnUIButtonClick()
        {
            ...
            if (!_isHighLightOn) // 하이라이트가 꺼져 있을 때
            {
                ...
                if(HighLightEventSystem.CurrentCanPlaceType != _canPlaceType) // 만약 현재 배치 가능한 타입이 다르다면
                {
                    ...
                    HighLightEventSystem.OnPieceHighLightUIAction?.Invoke(true, true); // 배치 가능한 기물 칸 하이라이트 키기(하이라이트 키기 여부, 배치 칸 이동 칸 여부 - true는 배치칸, false는 이동칸)
            ...
            else // 하이라이트가 켜져있을 때
            {
                ...
                {
                    HighLightEventSystem.OnPieceHighLightUIAction?.Invoke(false, true); // 배치 가능한 기물 칸 하이라이트 키기(하이라이트 키기 여부, 배치 칸 이동 칸 여부 - true는 배치칸, false는 이동칸)
