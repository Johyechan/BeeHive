        ...
        public override async void OnUIClick()
        {
            await MainThreadDispatcher.Enqueue(async () =>
            {
                ...
                if (!_isHighLightOn) // 하이라이트가 꺼져있을 때
                {
                    ...
                    NetworkManager.Instance.Socket.Emit("debug", $"도로 버튼 클릭, 길이:{PlacePlaneManager.Instance.Variable.highLightHandler.CanRoadPlacePlanes.Count}");
                    ...
                    NetworkManager.Instance.Socket.Emit("debug", "하이라이트 킴");
