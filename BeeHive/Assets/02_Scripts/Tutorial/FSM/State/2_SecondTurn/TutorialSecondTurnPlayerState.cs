            ...
            if(TutorialManager.Instance.TurnEnd) // 현재 턴이 종료 되었을 때
            {
                ...
                switch(InGameContext.Current.Data.TurnManager.CurrentTurnType) // 현재 턴이
                {
                    ...
                    case TurnType.MakeTurn: // 생산 턴일 때
                        string minerGetGoldCoin = LocalizationSettings.StringDatabase.GetLocalizedString(
                            "Tutorial",
                            "Tutorial_MinerGetGoldCoin"
                        );
                        TutorialManager.Instance.SetTutorialPanel(true, minerGetGoldCoin, TutorialManager.Instance.EnterClick, 0.08f, 0.008f, new Vector4(0.17f, 0.37f), new Vector4(0.8f, 0.3f));
